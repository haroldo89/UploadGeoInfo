
// Code to find the distance in metres between a lat/lng point and a polyline of lat/lng points
// All in WGS84. Free for any use.
//
// Bill Chadwick 2007
// updated to Google Maps API v3, Lawrence Ross 2014

// Construct a GeoUtils from its latitude and longitude in degrees
function GeoUtils(lat, lon) {
    var theta = (lon * Math.PI / 180.0);
    var rlat = GeoUtilsGeocentricLatitude(lat * Math.PI / 180.0);
    var c = Math.cos(rlat);
    this.x = c * Math.cos(theta);
    this.y = c * Math.sin(theta);
    this.z = Math.sin(rlat);
}
GeoUtils.prototype = new GeoUtils();

// internal helper functions =========================================

// Convert from geographic to geocentric latitude (radians).
function GeoUtilsGeocentricLatitude(geographicLatitude) {
    var flattening = 1.0 / 298.257223563;//WGS84
    var f = (1.0 - flattening) * (1.0 - flattening);
    return Math.atan((Math.tan(geographicLatitude) * f));
}

// Convert from geocentric to geographic latitude (radians)
function GeoUtilsGeographicLatitude(geocentricLatitude) {
    var flattening = 1.0 / 298.257223563;//WGS84
    var f = (1.0 - flattening) * (1.0 - flattening);
    return Math.atan(Math.tan(geocentricLatitude) / f);
}

// Returns the two antipodal points of intersection of two great
// circles defined by the arcs geo1 to geo2 and
// geo3 to geo4. Returns a point as a Geo, use .antipode to get the other point
function GeoUtilsGetIntersection(geo1, geo2, geo3, geo4) {
    var geoCross1 = geo1.crossNormalize(geo2);
    var geoCross2 = geo3.crossNormalize(geo4);
    return geoCross1.crossNormalize(geoCross2);
}

//from Radians to Meters
function GeoUtilsRadiansToMeters(rad) {
    return rad * 6378137.0; // WGS84 Equatorial Radius in Meters
}

//from Meters to Radians
function GeoUtilsMetersToRadians(m) {
    return m / 6378137.0; // WGS84 Equatorial Radius in Meters
}

// properties =================================================


GeoUtils.prototype.getLatitudeRadians = function () {
    return (GeoUtilsGeographicLatitude(Math.atan2(this.z,
        Math.sqrt((this.x * this.x) + (this.y * this.y)))));
}

GeoUtils.prototype.getLongitudeRadians = function () {
    return (Math.atan2(this.y, this.x));
}

GeoUtils.prototype.getLatitude = function () {
    return this.getLatitudeRadians() * 180.0 / Math.PI;
}

GeoUtils.prototype.getLongitude = function () {
    return this.getLongitudeRadians() * 180.0 / Math.PI;
}

// Methods =================================================

//Maths
GeoUtils.prototype.dot = function (b) {
    return ((this.x * b.x) + (this.y * b.y) + (this.z * b.z));
}

//More Maths
GeoUtils.prototype.crossLength = function (b) {
    var x = (this.y * b.z) - (this.z * b.y);
    var y = (this.z * b.x) - (this.x * b.z);
    var z = (this.x * b.y) - (this.y * b.x);
    return Math.sqrt((x * x) + (y * y) + (z * z));
}

//More Maths
GeoUtils.prototype.scale = function (s) {
    var r = new GeoUtils(0, 0);
    r.x = this.x * s;
    r.y = this.y * s;
    r.z = this.z * s;
    return r;
}

// More Maths
GeoUtils.prototype.crossNormalize = function (b) {
    var x = (this.y * b.z) - (this.z * b.y);
    var y = (this.z * b.x) - (this.x * b.z);
    var z = (this.x * b.y) - (this.y * b.x);
    var L = Math.sqrt((x * x) + (y * y) + (z * z));
    var r = new GeoUtils(0, 0);
    r.x = x / L;
    r.y = y / L;
    r.z = z / L;
    return r;
}

// point on opposite side of the world to this point
GeoUtils.prototype.antipode = function () {
    return this.scale(-1.0);
}

//distance in radians from this point to point v2
GeoUtils.prototype.distance = function (v2) {
    return Math.atan2(v2.crossLength(this), v2.dot(this));
}

//returns in meters the minimum of the perpendicular distance of this point from the line segment geo1-geo2
//and the distance from this point to the line segment ends in geo1 and geo2 
GeoUtils.prototype.distanceToLineSegMtrs = function (geo1, geo2) {

    //point on unit sphere above origin and normal to plane of geo1,geo2
    //could be either side of the plane
    var p2 = geo1.crossNormalize(geo2);

    // intersection of GC normal to geo1/geo2 passing through p with GC geo1/geo2
    var ip = GeoUtilsGetIntersection(geo1, geo2, this, p2);

    //need to check that ip or its antipode is between p1 and p2
    var d = geo1.distance(geo2);
    var d1p = geo1.distance(ip);
    var d2p = geo2.distance(ip);
    //window.status = d + ", " + d1p + ", " + d2p;
    if ((d >= d1p) && (d >= d2p))
        return GeoUtilsRadiansToMeters(this.distance(ip));
    else {
        ip = ip.antipode();
        d1p = geo1.distance(ip);
        d2p = geo2.distance(ip);
    }
    if ((d >= d1p) && (d >= d2p))
        return GeoUtilsRadiansToMeters(this.distance(ip));
    else
        return GeoUtilsRadiansToMeters(Math.min(geo1.distance(this), geo2.distance(this)));
}

// distance in meters from GLatLng point to GPolyline or GPolygon poly
function GeoUtilsDistanceToPolyMtrs(poly, point) {
    var d = 999999999;
    var i;
    var p = new GeoUtils(point.lat(), point.lng());
    for (i = 0; i < (poly.getPath().getLength() - 1) ; i++) {
        var p1 = poly.getPath().getAt(i);
        var l1 = new GeoUtils(p1.lat(), p1.lng());
        var p2 = poly.getPath().getAt(i + 1);
        var l2 = new GeoUtils(p2.lat(), p2.lng());
        var dp = p.distanceToLineSegMtrs(l1, l2);
        if (dp < d)
            d = dp;
    }
    return d;
}

//Obtener distancia recorrida en ruta.
function GeoUtilsDistanceInTrip(poly, point) {
    //var routePolyline = GoogleMapsHelper.createPolyline({ path: [], geodesic: true });
    var d = 999999999999999;
    var polylinePath = poly.getPath();
    var totalPointPoly = poly.getPath().getLength();
    var distance = 0;
    var currentDistance = 0;
    var p = new GeoUtils(point.lat(), point.lng());
    for (var i = 0; i < (totalPointPoly - 1) ; i++) {
        var routePolyline = GoogleMapsHelper.createPolyline({ path: [], geodesic: true });
        var p1 = polylinePath.getAt(i);
        var l1 = new GeoUtils(p1.lat(), p1.lng());
        var p2 = polylinePath.getAt(i + 1);
        var l2 = new GeoUtils(p2.lat(), p2.lng());
        var dp = p.distanceToLineSegMtrs(l1, l2);
        routePolyline.getPath().push(polylinePath.getAt(i));
        routePolyline.getPath().push(polylinePath.getAt(i + 1));
        distance += google.maps.geometry.spherical.computeLength(routePolyline.getPath());
        if (dp < d) {
            d = dp;
            currentDistance = distance;
            //distance = google.maps.geometry.spherical.computeLength(routePolyline.getPath());
        }
    }
    return currentDistance;
}

//Obtener porcentaje de ruta
function GeoUtilsDistanceInTripPercent(poly, point) {
    var percentIntrip = 0;
    var totalPointPoly = google.maps.geometry.spherical.computeLength(poly.getPath());
    var distanceTrip = GeoUtilsDistanceInTrip(poly, point);
    percentIntrip = (distanceTrip / totalPointPoly) * 100;
    return percentIntrip;
}

////Obtener porcentaje de ruta
//function GeoUtilsDistanceInTripPercent(poly, point) {
//    var d = 999999999999999;
//    var percentIntrip = 0;
//    var position = 0
//    var totalPointPoly = poly.getPath().getLength();
//    var p = new GeoUtils(point.lat(), point.lng());
//    for (var i = 0; i < (totalPointPoly - 1) ; i++) {
//        var p1 = poly.getPath().getAt(i);
//        var l1 = new GeoUtils(p1.lat(), p1.lng());
//        var p2 = poly.getPath().getAt(i + 1);
//        var l2 = new GeoUtils(p2.lat(), p2.lng());
//        var dp = p.distanceToLineSegMtrs(l1, l2);
//        position = i + 1;
//        if (dp < d) {
//            d = dp;
//            percentIntrip = (position / totalPointPoly) * 100;
//        }
//    }
//    return percentIntrip;
//}


// get a new GLatLng distanceMeters away on the compass bearing azimuthDegrees
// from the GLatLng point - accurate to better than 200m in 140km (20m in 14km) in the UK

function GeoUtilsPointAtRangeAndBearing(point, distanceMeters, azimuthDegrees) {
    var latr = point.lat() * Math.PI / 180.0;
    var lonr = point.lng() * Math.PI / 180.0;

    var coslat = Math.cos(latr);
    var sinlat = Math.sin(latr);
    var az = azimuthDegrees * Math.PI / 180.0;
    var cosaz = Math.cos(az);
    var sinaz = Math.sin(az);
    var dr = distanceMeters / 6378137.0; // distance in radians using WGS84 Equatorial Radius
    var sind = Math.sin(dr);
    var cosd = Math.cos(dr);

    return new google.maps.LatLng(Math.asin((sinlat * cosd) + (coslat * sind * cosaz)) * 180.0 / Math.PI,
    (Math.atan2((sind * sinaz), (coslat * cosd) - (sinlat * sind * cosaz)) + lonr) * 180.0 / Math.PI);
}
